...
func myRubato(rubatoParam int, paramIndex int, radix int, fullCoeffs bool) {
	var err error
	...
	var data [][]float64
	// var flattenWeights []float64  # let's try with a vector len 100352 (128 * 784)
	var nonces [][]byte
	var counter []byte
	var key []uint64
	var keystream [][]uint64
	...
	// Rubato parameter
	fmt.Println("--- Rubato parameter ---")
	blocksize := ckks_fv.RubatoParams[rubatoParam].Blocksize
	numRound := ckks_fv.RubatoParams[rubatoParam].NumRound
	plainModulus := ckks_fv.RubatoParams[rubatoParam].PlainModulus
	sigma := ckks_fv.RubatoParams[rubatoParam].Sigma
	fmt.Printf("blocksize = %d\n", blocksize)
	fmt.Printf("numRound = %d\n", numRound)
	fmt.Printf("plainModulus = %d\n", plainModulus)
	fmt.Printf("sigma = %f\n", sigma)
	...
	*/
	...
	fmt.Printf("params.Slots() = %d\n", params.Slots())
	fmt.Printf("params.N() = %d\n", params.N())
	fmt.Printf("params.LogN() = %d\n", params.LogN())
	params.SetPlainModulus(plainModulus)
	messageScaling := float64(params.PlainModulus()) / (2 * hbtpParams.MessageRatio)
	fmt.Printf("messageScaling = %f\n", messageScaling)

	// Rubato parameters in RtF
	rubatoModDown = make([]int, numRound)
	stcModDown = make([]int, 30)
	fmt.Printf("len(rubatoModDown) = %d\n", len(rubatoModDown))
	fmt.Printf("len(stcModDown) = %d\n", len(stcModDown))

	// fullCoeffs (default to false) denotes whether full coefficients are used for data encoding
	if fullCoeffs {
		params.SetLogFVSlots(params.LogN())
	} else {
		params.SetLogFVSlots(params.LogSlots())
	...
	// Scheme context and keys
	fmt.Println("\n--- Scheme context and keys generation ---")
	// kgen = ckks_fv.NewKeyGenerator(params)
	// sk, pk = kgen.GenKeyPairSparse(hbtpParams.H)
	...
	// Generating half-bootstrapping keys
	fmt.Printf("Generating half-bootstrapping keys\n")
	// rotationsHalfBoot := kgen.GenRotationIndexesForHalfBoot(params.LogSlots(), hbtpParams)
	// pDcds := fvEncoder.GenSlotToCoeffMatFV(radix)
	// rotationsStC := kgen.GenRotationIndexesForSlotsToCoeffsMat(pDcds)
	// rotations := append(rotationsHalfBoot, rotationsStC...)
	...
	// Encode float data added by keystream to plaintext coefficients
	// fvEvaluator = ckks_fv.NewMFVEvaluator(para`ms, ckks_fv.EvaluationKey{Rlk: rlk, Rtks: rotkeys}, pDcds)
	// params.Slots() = numCols
	// outputsize = numRows
	numCols, numRows := 784, 128
	outputsize := blocksize - 4 // 32
	...
	fmt.Printf("Data dimensions: [%d, %d]\n", len(data), len(data[0]))

	fmt.Printf("\n--- Client (Offline Phase): Creating some nonces (not using fullCoeffs) ---\n")
	nonces = make([][]byte, numCols)
	for i := 0; i < numCols; i++ {
		nonces[i] = make([]byte, 64)
		rand.Read(nonces[i])
	}
	...
	keystream = make([][]uint64, numCols)
	for i := 0; i < numCols; i++ {
		keystream[i] = plainRubato(
			blocksize, numRound, nonces[i], counter, key, params.PlainModulus(), sigma,
		)
	}
	fmt.Printf("keystream: [%d, %d]\n", len(keystream), len(keystream[0]))

	for s := 0; s < numRows; s++ {
		for i := 0; i < numCols/2; i++ {
			j := utils.BitReverse64(uint64(i), uint64(params.LogN()-1))
			coeffs[s][j] = data[s][i]
			coeffs[s][j+uint64(numCols/2)] = data[s][i+numCols/2]
		}
	}
...
func myRubato2(rubatoParam int, paramIndex int, radix int, fullCoeffs bool) {
	var err error
	...
	fmt.Printf("Time to create boostrapping keys: %v\n", end.Sub(start))
	fmt.Printf("params.Slots(): %d\n", params.Slots())
	...
	fmt.Print("\n--- Client (Offline Phase): Generating keystream ---\n")
	keystream = make([][]uint64, numCols)
	for i := 0; i < numCols; i++ {
		keystream[i] = plainRubato(blocksize, numRound, nonces[i], counter, key, params.PlainModulus(), sigma)
	}
	...
	for s := 0; s < outputsize; s++ {
		for i := 0; i < numCols/2; i++ {
			j := utils.BitReverse64(uint64(i), uint64(params.LogN()-1))
			coeffs[s][j] = data[s][i]
			coeffs[s][j+uint64(params.N()/2)] = data[s][i+numCols/2]
		}
	}

	fmt.Println("\n--- Client (Online Phase): Handling plaintext encoding and scaling ---")
	plainCKKSRingTs = make([]*ckks_fv.PlaintextRingT, outputsize)
	for s := 0; s < outputsize; s++ {
		plainCKKSRingTs[s] = ckksEncoder.EncodeCoeffsRingTNew(coeffs[s], messageScaling)
		poly := plainCKKSRingTs[s].Value()[0]
	...
	fmt.Printf("rubatoParam: %+v\n", rubatoParam)
	fmt.Printf("params: %+v\n", params)
	fmt.Printf("rubatoModDown[0]: %d\n", rubatoModDown[0])
	fmt.Printf("nonces length: %d\n", len(nonces))
	fmt.Printf("counter length: %d\n", len(counter))
	fmt.Printf("key length: %d\n", len(key))
	fmt.Printf("stcModDown: %+v\n", stcModDown)

	rubato = ckks_fv.NewMFVRubato(rubatoParam, params, fvEncoder, fvEncryptor, fvEvaluator, rubatoModDown[0])
	kCt := rubato.EncKey(key)

	// FV Keystream
	fmt.Printf("\n--- Client (Online Phase): Generating FV Keystream ---\n")
	fvKeystreams = rubato.CryptNoModSwitch(nonces, counter, kCt)
	fmt.Printf("\n--- Client (Online Phase): Generating FV Keystream (end) ---\n")
	for i := 0; i < outputsize; i++ {
		fvKeystreams[i] = fvEvaluator.SlotsToCoeffs(fvKeystreams[i], stcModDown)
		fvEvaluator.ModSwitchMany(fvKeystreams[i], fvKeystreams[i], fvKeystreams[i].Level())
	}
	fmt.Printf("fvKeystreams levels: %v\n", fvKeystreams[0].Level())

	fmt.Printf("\n--- Client (Online Phase): Half-Bootstrapping ---\n")
	var ctBoot *ckks_fv.Ciphertext
	for s := 0; s < outputsize; s++ {
		// Encrypt and mod switch to the lowest level
		ciphertext := ckks_fv.NewCiphertextFVLvl(params, 1, 0)
		...
		valuesWant := make([]complex128, numCols)
		for i := 0; i < numCols; i++ {
			valuesWant[i] = complex(data[s][i], 0)
		}
